...
| `SAKURA_ACCESS_TOKEN`          | 条件付き | -         | Sakura Cloud API トークン。`SAKURA_VAULT_ID` が設定された場合は必須。                                                   |
| `SAKURA_ACCESS_TOKEN_SECRET`   | 条件付き | -         | Sakura Cloud API トークンシークレット。`SAKURA_VAULT_ID` が設定された場合は必須。                                      |
...
#### 方法 1: Sakura Cloud Secret Manager を使用（推奨）

Terraform でインフラを構築後、以下の環境変数を設定します：

```bash
# Terraform の出力から取得
export SAKURA_VAULT_ID=$(terraform output -raw database_secret_vault_id)
export SAKURA_SECRET_NAME=$(terraform output -raw database_secret_name)

# Sakura Cloud API 認証情報（Sakura Cloud Console から取得）
export SAKURA_ACCESS_TOKEN="your-api-token"
export SAKURA_ACCESS_TOKEN_SECRET="your-api-token-secret"

export ALLOWED_ORIGIN="https://w32hlqto.user.webaccel.jp"
export PORT=8080
```

> **注**: `SAKURA_ACCESS_TOKEN` と `SAKURA_ACCESS_TOKEN_SECRET` は sakura-api-go ライブラリによって自動的に読み込まれます。

#### 方法 2: 従来の環境変数を使用

```bash
export DB_DSN="user:password@tcp(db.example.com:3306)/movie_schedule"
export ALLOWED_ORIGIN="https://w32hlqto.user.webaccel.jp"
export PORT=8080
```
...
MariaDB 10.11 以上推奨。`table.sql` でスキーマを初期化してください。
