...
// ValidateCastAvailability はキャスト参加可否の入力値を検証します
func ValidateCastAvailability(c models.CastAvailability) error {
	if c.IsAvailable != 0 && c.IsAvailable != 1 {
		return ValidationError{Field: "is_available", Message: "参加可否は0または1で入力してください。"}
	}
	return nil
}
